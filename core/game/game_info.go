...
func (g *GameInfo) ExtraMap() map[string]any {
	return g.Extra
}
func LoadGameConfig(filePath string) (*GameInfos, error) {
...
func getNested[T any](m map[string]any, path ...string) T {
	...
	for _, p := range path {
		if currMap, ok := curr.(map[string]any); ok {
			...
			panic(fmt.Sprintf("No corresponding configuration: %v", path))
		}
	}
	t, ok := curr.(T)
	if !ok {
		panic(fmt.Sprintf("config type error: expected %T at path %v, got %T (value=%v)", *new(T), path, curr, curr))
	}
	return t
}
